...
export const ZkValidatorRules = {
    ...
    required: {
        handler: function (element) {
            ...
            if (['checkbox', 'radio'].includes(element.type)) {
                ...
                if (element.multiple) {
                    return Array.from(element.options).some(option => option.selected && option.value.trim() !== '');
                }
            }
    ...
    email: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || element.value.match(/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/);
    ...
    numeric: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || element.value.match(/^-?\d*\.?\d+$/);
        },
        message: function (element, message = '') {
    ...
    alphaNumeric: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || element.value.match(/^[a-zA-Z0-9]+$/);
        },
        message: function (element, message = '') {
    ...
    alphaDash: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || element.value.match(/^[a-zA-Z0-9_-]+$/);
        },
        message: function (element, message = '') {
    ...
    url: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || element.value.match(/^(http|https):\/\/[a-zA-Z0-9-\.]+\.[a-z]{2,4}/);
        },
        message: function (element, message = '') {
    ...
    date: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || element.value.match(/^\d{4}-\d{2}-\d{2}$/);
        },
        message: function (element, message = '') {
    ...
    boolean: {
        handler: function (element) {
            ...
            return element.value.trim() === '' || ['true', 'false', '1', '0'].includes(element.value);
        },
        message: function (element, message = '') {
    ...
    pattern: {
        handler: function (element, pattern) {
            ...
            return element.value.trim() === '' || element.value.match(new RegExp(pattern));
        },
        message: function (element, message = '') {
    ...
    between: {
        handler: function (element, min, max) {
            ...
            return element.value.trim() === '' || (parseFloat(element.value) >= min && parseFloat(element.value) <= max);
        },
    ...
    unique: {
        handler: async function (element, url, ...args) {
            ...
            _self.dispatchCustomEvent(element, 'zk-unique-loading', { url, body: body.toString() });
            ...
            try {
                ...
                _self.dispatchCustomEvent(element, 'zk-unique-success', { isValid: data.valid, url, body: body.toString() });
                return data.valid;
            } catch (error) {
                _self.dispatchCustomEvent(element, 'zk-unique-error', { error, url, body: body.toString() });
                return false;
            } finally {
                _self.dispatchCustomEvent(element, 'zk-unique-complete', { url, body: body.toString() });
...
export class ZkFormValidator {
    ...
    initFieldSelector() {
        if (this.options.exclude.length) {
            const inputNotSelector = [], attrNotSelector = [];
            ...
            this.fieldSelector = this.fieldSelector.split(', ').map(field => {
                return (field === 'input')
    ...
    registerFieldEvents(element, rules, messages) {
        const events = ['checkbox', 'radio'].includes(element.type) ? ['click', 'change'] : ['change', 'keyup'];
        const listener = async (event) => {
            await this.validateField(element, rules, messages);
        };
        events.forEach(event => {
            const _listener = event === 'keyup' ? this.debounce(listener, 500) : listener;
    ...
    async validateField(element, rules, messages = {}) {
        const isValidField = await this.validator.validateField(element, rules, messages);
    ...
    async validate(fields = {}) {
        ...
        for (const name in _fields) {
            ...
            const element = this.getElement(name);
            if (element) {
                const isValidField = await this.validateField(element, field.rules || field, field.messages || {});
                if (!isValidField) this.isValid = false;
            }
        }

        if (!this.isValid) {
            const focusElement = this.form.querySelector(`.${this.options.input.errorClass}`);
    ...
    showError(element, errors) {
        ...
        if (this.options.input.successClass) element.classList.remove(this.options.input.successClass);
        if (this.options.inputGroup.selector) element.closest(this.options.inputGroup.selector)?.classList.remove(this.options.inputGroup.successClass);

        let errorElement = this.options.error.group && this.options.error.group.tag
        ...
        for (const name in errors) {
            ...
            errorElement ? errorElement.appendChild(error) : element.insertAdjacentElement(this.options.position, error);
        }
        if (this.options.input.errorClass) element.classList.add(this.options.input.errorClass);
        if (this.options.inputGroup.selector) element.closest(this.options.inputGroup.selector)?.classList.add(this.options.inputGroup.errorClass);

        errorElement && element.insertAdjacentElement(this.options.position, errorElement);
    }

    validField(element) {
        this.clearError(element);
        if (this.options.input.successClass) element.classList.add(this.options.input.successClass);
        if (this.options.inputGroup.selector) element.closest(this.options.inputGroup.selector)?.classList.add(this.options.inputGroup.successClass);
    }

    clearError(element) {
        if (this.options.input.errorClass) element.classList.remove(this.options.input.errorClass);
        if (this.options.inputGroup.selector) element.closest(this.options.inputGroup.selector)?.classList.remove(this.options.inputGroup.errorClass);

        const siblingElements = [element.nextElementSibling];
        if (this.options.error.group && this.options.error.group.tag) {
            if (siblingElements[0] && siblingElements[0].classList.contains(this.options.error.group.class)) {
                siblingElements[0].remove();
            }
        } else {
            while (siblingElements[0] && siblingElements[0].classList.contains(this.options.error.class)) {
                siblingElements[0].remove();
    ...
    clearErrors() {
        ...
        if (this.options.error.group.class) this.form.querySelectorAll(`.${this.options.error.group.class}`).forEach(el => el.remove());
        if (this.options.error.class) this.form.querySelectorAll(`.${this.options.error.class}`).forEach(el => el.remove());
        if (this.options.input.errorClass) this.form.querySelectorAll(`.${this.options.input.errorClass}`).forEach(el => el.classList.remove(this.options.input.errorClass));
        if (this.options.input.successClass) this.form.querySelectorAll(`.${this.options.input.successClass}`).forEach(el => el.classList.remove(this.options.input.successClass));
        if (this.options.inputGroup.errorClass) this.form.querySelectorAll(`.${this.options.inputGroup.errorClass}`).forEach(el => el.classList.remove(this.options.inputGroup.errorClass));
        if (this.options.inputGroup.successClass) this.form.querySelectorAll(`.${this.options.inputGroup.successClass}`).forEach(el => el.classList.remove(this.options.inputGroup.successClass));
    }
...
export default class ZkValidator {
        ...
        for (const rule in rules) {
            ...
            if (!ruleResult) {
                ...
                const message = (this.rules[rule].message && typeof this.rules[rule].message === 'function')
                    ...
                    : _message;
                this.errors[element.name] = { ...(this.errors[element.name] || {}), [rule]: message };
    ...
    async validate(fields = {}) {
        ...
        for (const name in _fields) {
            ...
            if (!element) continue;
            const field = _fields[name];
            const fieldValid = await this.validateField(element, field.rules || field, field.messages || {});
    ...
    async executeRule(rule, element, args) {
        ...
        return (result && typeof result.then === 'function' || result instanceof Promise) ? await result : result;
    }
